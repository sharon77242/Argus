...
import {
  ...
  type QueryLike,
  type HttpLike,
} from "../../src/analysis/circuit-breaker-detector.ts";
...
describe("CircuitBreakerDetector", () => {
  ...
  test("does not fire when below minCalls (only 5 calls)", () => {
    ...
    const events: QueryLike[] = [...Array.from({ length: 5 }, () => makeQuery({ error: true }))];
  ...
  test("resets after window slides past failure window", () => {
    ...
    const oldEvents: QueryLike[] = Array.from(
      { length: 10 },
  ...
  test("suggestion includes suggestedFix with circuit-breaker pattern example", () => {
    ...
    assert.ok(
      suggestions[0].suggestedFix.includes("circuit breaker"),
      "suggestedFix should mention circuit breaker",
    );
    assert.ok(
      suggestions[0].suggestedFix.includes("opossum"),
      "suggestedFix should include a library example",
    );
  });
