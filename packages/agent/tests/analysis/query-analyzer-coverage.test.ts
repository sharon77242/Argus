...
describe("QueryAnalyzer (coverage)", () => {
  ...
  it("should fall back to string analysis with UPDATE that HAS WHERE — no flag", () => {
    ...
    assert.strictEqual(
      rule,
      undefined,
      "Should not flag UPDATE WITH WHERE even in string fallback",
    );
  });
  ...
  it("reset() should clear recent query tracking so N+1 restarts", () => {
    ...
    for (let i = 0; i < 4; i++) {
      const s = analyzer.analyze(q);
      assert.ok(
        !s.find((x) => x.rule === "n-plus-one"),
        `Unexpected N+1 on call ${i + 1} after reset`,
      );
    }
  });
  ...
  it("[BUG FIX] N+1 warning should fire on 6th and 7th call, not just the 5th", () => {
    ...
    assert.ok(
      s5.find((x) => x.rule === "n-plus-one"),
      "Should warn on 5th call",
    );

    // Call 6: should ALSO warn (old === bug would silently skip this)
    const s6 = analyzer.analyze(q);
    assert.ok(
      s6.find((x) => x.rule === "n-plus-one"),
      "[BUG FIX] Should still warn on 6th call",
    );

    // Call 7: same
    const s7 = analyzer.analyze(q);
    assert.ok(
      s7.find((x) => x.rule === "n-plus-one"),
      "[BUG FIX] Should still warn on 7th call",
    );
  });
  ...
  it("[FIX] warned flag should reset when window expires, so first-spike message fires again", async () => {
    ...
    assert.ok(
      firstSpike.find((x) => x.rule === "n-plus-one"),
      "Sanity: should warn on 5th call",
    );
    assert.ok(
    ...
    assert.ok(
      warn!.message.includes("detected"),
