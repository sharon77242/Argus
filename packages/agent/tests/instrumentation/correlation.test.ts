...
import { runWithContext, getCurrentContext, createRequestContext } from '../../src/instrumentation/correlation.ts';
...
describe('Correlation IDs', () => {
  ...
  test('context is isolated between concurrent requests', async () => {
    ...
    const results: Array<{ url: string | undefined; requestId: string }> = [];
  ...
  test('createRequestContext captures startedAt as approximate current time', () => {
    ...
    assert.ok(ctx.startedAt >= before && ctx.startedAt <= after, 'startedAt should be current time');
  });

  test('context does not bleed out after runWithContext completes', () => {
    const ctx = createRequestContext();
    runWithContext(ctx, () => { /* no-op */ });
    const leaked = getCurrentContext();
    assert.equal(leaked, undefined, 'context should not be accessible after runWithContext completes');
  });
});
