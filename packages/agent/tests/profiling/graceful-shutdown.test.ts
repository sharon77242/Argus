...
function makeAgentStub() {
  const agent = new EventEmitter() as EventEmitter & { stop: () => Promise<void>; stopCallCount: number };
  agent.stopCallCount = 0;
  agent.stop = async () => { agent.stopCallCount++; };
...
describe('GracefulShutdown', () => {
  test('registers SIGTERM and SIGINT handlers', () => {
    ...
    assert.equal(process.listenerCount('SIGTERM'), sigterm0 + 1, 'SIGTERM listener should be added');
  ...
  test('calling register() twice is idempotent (no duplicate handlers)', () => {
    ...
    assert.equal(process.listenerCount('SIGTERM'), sigterm0 + 1, 'Only one SIGTERM handler should be registered');
    assert.equal(process.listenerCount('SIGINT'), sigint0 + 1, 'Only one SIGINT handler should be registered');
  ...
  test('SIGTERM: emits info message, calls stop(), exits 0 on happy path', async () => {
    await withExitMock(async (exitCodes) => {
      ...
      assert.ok(messages.some(m => m.includes('SIGTERM')), 'Should emit info about SIGTERM');
  ...
  test('SIGINT: emits info message, calls stop(), exits 0 on happy path', async () => {
    await withExitMock(async (exitCodes) => {
      ...
      assert.ok(messages.some(m => m.includes('SIGINT')), 'Should emit info about SIGINT');
      assert.equal(agent.stopCallCount, 1, 'agent.stop() should be called once');
      assert.deepEqual(exitCodes, [0], 'Should exit with code 0 on clean flush');
    });
  });
  ...
  test('listener throwing on info event does not prevent shutdown', async () => {
    await withExitMock(async (exitCodes) => {
      const agent = makeAgentStub();
      (agent as unknown as EventEmitter).on('info', () => { throw new Error('bad listener'); });
