...
describe('validateLicense', () => {
  ...
  test('expired JWT throws with message EXPIRED', () => {
    ...
    assert.throws(() => validateLicense(jwt), (err: Error) => {
  ...
  test('sub claim is opaque — not a raw email or UUID', () => {
    const jwt = buildJwt(validClaims);
    const claims = validateLicense(jwt);
    // sub should be a short hex-like hash, not an email or UUID
    assert.ok(!claims.sub.includes('@'), 'sub should not be an email');
    assert.ok(!claims.sub.includes('-') || claims.sub.length < 36, 'sub should not be a UUID');
  });
